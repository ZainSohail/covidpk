...
class PakistanSats extends Component {
  ...
  render() {
    const { error, isLoaded, data, dbdata } = this.state;

    if ( dbdata !== null ) {
      console.log(dbdata.total_cases)
    }
    ...
    if ( data !== undefined ) {
        ...
        if ( record !== undefined ) {
          ...
          critical = record.cases.filter(function (index) {
              if ( index.name === "Critical" ) 
                return index.stat;
          });


          recovered = record.cases.filter(function (index) {
              if ( index.name === "Recovered" ) 
                return index.stat;
          });

          deaths = record.cases.filter(function (index) {
              if ( index.name === "Deaths" ) 
                return index.stat;
          });

          newcases = record.cases.filter(function (index) {
              if ( index.name === "Cases (24 HRS)" ) 
                return index.stat;
          });

          currentActive = record.total - recovered[0].stat - deaths[0].stat;
                 ...
                 <div className="card">
                    <div className="card-body">
                       ...
                       <p className="card-text text-left" >
                          <CountUp end={ dbdata ? dbdata.total_cases : 0 } />
